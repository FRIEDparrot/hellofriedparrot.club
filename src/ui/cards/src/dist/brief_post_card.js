"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
var vue_1 = require("vue");
var drafts_api_1 = require("@/api/blogs/drafts_api");
exports["default"] = vue_1.defineComponent({
    name: "BriefPostCards",
    props: {},
    data: function () {
        return {
            small_width_layout: vue_1.ref(false),
            showJumpInput: false,
            jumpPage: 0,
            currentPage: 1,
            totalPages: 5,
            handleJumpPage: function () { },
            post_card: {
                tabs: 0,
                loading: false,
                itemsPerPage: 5,
                itemsContent: {},
                items: [
                    {
                        key: "history",
                        icon: "mdi-clock-outline",
                        title: this.$t("common.post_card.history"),
                        content: [],
                        currentPage: 1
                    },
                    {
                        key: "draft",
                        icon: "mdi-pencil",
                        title: this.$t("common.post_card.drafts"),
                        content: [],
                        currentPage: 1
                    },
                    {
                        key: "stars",
                        icon: "mdi-star-outline",
                        title: this.$t("common.post_card.starFolder"),
                        content: [],
                        currentPage: 1
                    },
                    {
                        key: "posts",
                        icon: "mdi-text-box-multiple-outline",
                        title: this.$t("common.post_card.postFolder"),
                        content: [],
                        currentPage: 1
                    },
                ]
            }
        };
    },
    methods: {
        fetch_draft_data: function (params) {
            return __awaiter(this, void 0, void 0, function () {
                var idx, response, tableData, error_1;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            idx = this.post_card.items
                                .map(function (item) { return item.key; })
                                .indexOf("draft");
                            _a.label = 1;
                        case 1:
                            _a.trys.push([1, 3, , 4]);
                            return [4 /*yield*/, drafts_api_1.getUserDraftList(params)];
                        case 2:
                            response = _a.sent();
                            console.log(response);
                            tableData = response.tableData;
                            return [3 /*break*/, 4];
                        case 3:
                            error_1 = _a.sent();
                            return [3 /*break*/, 4];
                        case 4: return [2 /*return*/];
                    }
                });
            });
        },
        fetch_card_data_all: function () {
            return __awaiter(this, void 0, void 0, function () {
                var params;
                return __generator(this, function (_a) {
                    params = {
                        pageNum: 1,
                        itemsPerPage: this.post_card.itemsPerPage
                    };
                    return [2 /*return*/];
                });
            });
        },
        handleTabScroll: function (event) {
            // get the scroll direction (deltaY represents the vertical scroll,
            // positive means downward, negative means upward)
            var delta = Math.sign(event.deltaY);
            var nextIndex = this.post_card.tabs + delta;
            if (nextIndex < 0) {
                nextIndex = 0;
            }
            else if (nextIndex >= this.post_card.items.length) {
                nextIndex = this.post_card.items.length - 1;
            }
            this.post_card.tabs = nextIndex;
        },
        handleResize: function () {
            var card = this.$refs.card;
            if (card) {
                var cardWidth = card.$el.getBoundingClientRect().width;
                this.small_width_layout = cardWidth < 400;
            }
        }
    },
    mounted: function () {
        this.fetch_card_data_all();
        this.handleResize();
        window.addEventListener("resize", this.handleResize);
    },
    beforeUnmount: function () {
        window.removeEventListener("resize", this.handleResize);
    }
});
