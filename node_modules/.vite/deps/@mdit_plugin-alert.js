...
// node_modules/@mdit/plugin-alert/lib/index.js
var N = /^>(?:(?: {0,3})| {0,2}\t {0,1})\[!(.*)\]\s*$/i;
var O = (i, { alertNames: A = ["tip", "warning", "caution", "important", "note"], deep: v = false, openRender: M, closeRender: I, titleRender: y } = {}) => {
  const R = (e, s, o, T) => {
    if (e.sCount[s] - e.blkIndent >= 4 || e.level !== 0 && !v) return false;
    const S = e.bMarks[s] + e.tShift[s], $ = e.eMarks[s];
    if (e.src.charAt(S) !== ">") return false;
    const f = N.exec(e.src.slice(S, $));
    if (!f || !A.includes(f[1].toLowerCase())) return false;
    if (T) return true;
    const k = f[1].toLowerCase(), p = [], b = [], h = [], u = [], q = e.lineMax, J = e.parentType, L = e.md.block.ruler.getRules("alert");
    e.parentType = "alert";
    let t;
    for (t = s + 1; t < o; t++) {
      let r = false, x = false, a = e.bMarks[t] + e.tShift[t];
      const m = e.eMarks[t], E = e.sCount[t] < e.blkIndent;
      if (a >= m) break;
      if (e.src.charAt(a++) === ">" && !E) {
        let l, c = e.sCount[t] + 1;
        e.src.charAt(a) === " " ? (a++, c++, r = false, l = true) : e.src.charAt(a) === "	" ? (l = true, (e.bsCount[t] + c) % 4 === 3 ? (a++, c++, r = false) : r = true) : l = false;
        let d = c;
      ...
      if (x) break;
      let _ = false;
      for (const l of L) if (l(e, t, o, true)) {
        _ = true;
        break;
      }
      if (_) {
    ...
    return e.blkIndent = j, true;
  };
  i.block.ruler.before("blockquote", "alert", R, { alt: ["paragraph", "reference", "blockquote", "list"] }), M && (i.renderer.rules.alert_open = M), I && (i.renderer.rules.alert_close = I), i.renderer.rules.alert_title = y ?? ((e, s) => {
    const o = e[s];
...
//# sourceMappingURL=@mdit_plugin-alert.js.map
