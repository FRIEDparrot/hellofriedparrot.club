...
exports.Container = (md, name, options) => {
    ...
    const container = (state, startLine, endLine, silent) => {
        ...
        //
        if (marker_char !== state.src.charCodeAt(d.start))
            return false;
        ...
        for (;;) {
            ...
            if (marker_char !== state.src.charCodeAt(d.start))
                continue;
            if (state.sCount[d.nextLine] - state.blkIndent >= 4)
                continue;
            ...
            if (Math.floor((d.pos - d.start) / marker_len) < d.marker_count)
                continue;
